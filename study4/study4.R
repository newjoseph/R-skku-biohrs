library(haven);library(data.table);library(magrittr);library(fst);library(openxlsx);library(parallel)

setwd("/home/minhyuk.kim/ShinyApps/R-skku-biohrs/study4")
...
# g1e_obj_2018 <- read.csv("/home/minhyuk.kim/knhis_data/g1e_obj_2018.csv") %>% as.data.table()

# gq_rst_2018 <- read.csv("/home/minhyuk.kim/knhis_data/g1e_rst_2018.csv") %>% as.data.table()
#g1eq_2017 <- read.csv("/home/minhyuk.kim/knhis_data/g1eq_2017.csv") %>% as.data.table()

g1e_rst_2018 <- read.csv("/home/minhyuk.kim/knhis_data/g1e_rst_2018.csv") %>% as.data.table()
# g1eq_0217 <- read.csv("/home/minhyuk.kim/knhis_data/g1eq_0217.csv") %>% as.data.table()
g1eq_02 <- read.csv("/home/minhyuk.kim/knhis_data/g1eq_2002.csv") %>% as.data.table()

alist1 <- parallel::mclapply(2002:2017, \(x){read.csv(paste0("/home/minhyuk.kim/knhis_data/g1eq_", x, ".csv")) %>% data.table})
alist2 <- parallel::mclapply(2018:2021, \(x){read.csv(paste0("/home/minhyuk.kim/knhis_data/g1e_rst_", x, ".csv")) %>% data.table})
# alist2 <- parallel::mclapply(2009:2015, \(x){read.csv(paste0("/home/minhyuk.kim/knhis_data/g1eq_", x, ".csv")) %>% data.table})
# alist3 <- parallel::mclapply(2016:2017, \(x){read.csv(paste0("/home/minhyuk.kim/knhis_data/g1eq_", x, ".csv")) %>% data.table})

names(alist1) <- 2002:2017
names(alist2) <- 2018:2021
# names(alist3) <- 2016:2017
# names(alist4) <- 2018:2022

a1 <- alist1[as.character(c(2002:2008))] %>% rbindlist(fill = T)
a2 <- alist1[as.character(c(2009:2015))] %>% rbindlist(fill = T)
a3 <- alist1[as.character(c(2016:2017))] %>% rbindlist(fill = T)
#a4 <- alist2[as.character(c(2018:2021))] %>% rbindlist(fill = T)

# Q_DRK_FRQ_V0108,Q_DRK_AMT_V0108
# Q_DRK_FRQ_V09N, Q_DRK_AMT_V09N
Q_DRK

# a<- rbind(
# 
#   a1[, .(EXMD_BZ_YYYY, INDI_DSCM_NO, Q_DRK_FRQ = Q_DRK_FRQ_V0108, Q_DRK_AMT = Q_DRK_AMT_V0108)], #RN_INDI
#   a2[, .(EXMD_BZ_YYYY, INDI_DSCM_NO, Q_DRK_FRQ = Q_DRK_FRQ_V09N, Q_DRK_AMT = Q_DRK_AMT_V09N)], #RN_INDI
#   a3[, .(EXMD_BZ_YYYY, INDI_DSCM_NO, Q_DRK_FRQ = Q_DRK_FRQ_V09N, Q_DRK_AMT = Q_DRK_AMT_V09N)]
# 
# )

a0208 <-  a1[, .(EXMD_BZ_YYYY, INDI_DSCM_NO, Q_DRK_FRQ_V0108, Q_DRK_AMT_V0108)] #RN_INDI 
a0917 <- rbind( a[, .(EXMD_BZ_YYYY, INDI_DSCM_NO, Q_DRK_FRQ_V09N, Q_DRK_AMT_V09N)], #RN_INDI
                a3[, .(EXMD_BZ_YYYY, INDI_DSCM_NO, Q_DRK_FRQ_V09N, Q_DRK_AMT_V09N)])

gq_18 <-  fread(file.path(cpath,"gq_rst_2018.csv"))
gq_18 <- unique(gq_18[,.(INDI_DSCM_NO,HC_DT=HME_DT,HC_BZ_YYYY=EXMD_BZ_YYYY,
                         Q_DRK_PER,Q_DRK_FRQ,
                         ...
                         Q_PA_MD_MINS,Q_PA_MD_HRS,Q_PA_MD_FRQ,Q_PA_VD_MINS,Q_PA_VD_HRS,Q_PA_VD_FRQ
)])

gq_1922 <- fread(file.path(cpath,"gq_rst_1922.csv"))
gq_1922 <- unique(gq_1922[,.(INDI_DSCM_NO,HC_DT,HC_BZ_YYYY,
                             Q_DRK_PER,Q_DRK_FRQ,
                             Q_DRK_SOJU_SHOT,Q_DRK_SOJU_BTL,Q_DRK_SOJU_CAN,Q_DRK_SOJU_CC,
                             Q_DRK_BEER_SHOT,Q_DRK_BEER_BTL,Q_DRK_BEER_CAN,Q_DRK_BEER_CC,
                             Q_DRK_LQR_SHOT,Q_DRK_LQR_BTL,Q_DRK_LQR_CAN,Q_DRK_LQR_CC,
...
a4[, .(EXMD_BZ_YYYY, INDI_DSCM_NO)]


##drink
#17
gq_17[,.(Q_DRK_FRQ_V09N,Q_DRK_AMT_V09N)]

gq_17[Q_DRK_FRQ_V09N==0]$Q_DRK_AMT_V09N <- 0

gq_17[,DRK_gram:= Q_DRK_FRQ_V09N * Q_DRK_AMT_V09N * 14]

#18~22
gq_18 <- mutate_at(gq_18, names(gq_18)[like(names(gq_18),"DRK")][c(-1,-2)], ~replace(., is.na(.), 0)) %>% data.table

gq_1922 <- mutate_at(gq_1922, names(gq_1922)[like(names(gq_1922),"DRK")][c(-1,-2)], ~replace(., is.na(.), 0)) %>% data.table()
...
gq_18$PER  <-  sapply(gq_18$Q_DRK_PER,g1e_PER)

gq_18[Q_DRK_PER==4&is.na(Q_DRK_FRQ)==T]$Q_DRK_FRQ <- 0

gq_1922[, PER := NA]

gq_1922$PER  <-  sapply(gq_1922$Q_DRK_PER,g1e_PER)

gq_1922[Q_DRK_PER==4&is.na(Q_DRK_FRQ)==T]$Q_DRK_FRQ <- 0


volume = c(50,360,180,1,
           200,640,355,1,
           40,360,355,1,
           230,750,350,1,
           120,700,250,1)

ach_vol = c(17,17,12,17,
            4.5,4.5,4.5,4.5,
            40,40,7,40,
            6,6,6,6,
            12,12,12,12)

gq_18[,DRK_onetime := Q_DRK_SOJU_SHOT*volume[1]*ach_vol[1]*0.785/100 +
        Q_DRK_SOJU_BTL*volume[2]*ach_vol[2]*0.785/100 +
        Q_DRK_SOJU_CAN*volume[3]*ach_vol[3]*0.785/100 + 
        Q_DRK_SOJU_CC*volume[4]*ach_vol[4]*0.785/100+
        Q_DRK_BEER_SHOT*volume[5]*ach_vol[5]*0.785/100 +
        Q_DRK_BEER_BTL*volume[6]*ach_vol[6]*0.785/100+
        Q_DRK_BEER_CAN*volume[7]*ach_vol[7]*0.785/100+
        Q_DRK_BEER_CC*volume[8]*ach_vol[8]*0.785/100+
        Q_DRK_LQR_SHOT*volume[9]*ach_vol[9]*0.785/100+
        Q_DRK_LQR_BTL*volume[10]*ach_vol[10]*0.785/100+
        Q_DRK_LQR_CAN*volume[11]*ach_vol[11]*0.785/100+
        Q_DRK_LQR_CC*volume[12]*ach_vol[12]*0.785/100+
        Q_DRK_KLQR_SHOT*volume[13]*ach_vol[13]*0.785/100+
        Q_DRK_KLQR_BTL*volume[14]*ach_vol[14]*0.785/100+
        Q_DRK_KLQR_CAN*volume[15]*ach_vol[15]*0.785/100+
        Q_DRK_KLQR_CC*volume[16]*ach_vol[16]*0.785/100+
        Q_DRK_WINE_SHOT*volume[17]*ach_vol[17]*0.785/100+
        Q_DRK_WINE_BTL*volume[18]*ach_vol[18]*0.785/100+
        Q_DRK_WINE_CAN*volume[19]*ach_vol[19]*0.785/100+
        Q_DRK_WINE_CC*volume[20]*ach_vol[20]*0.785/100]

gq_1922[,DRK_onetime := Q_DRK_SOJU_SHOT*volume[1]*ach_vol[1]*0.785/100 +
          Q_DRK_SOJU_BTL*volume[2]*ach_vol[2]*0.785/100 +
          Q_DRK_SOJU_CAN*volume[3]*ach_vol[3]*0.785/100 + 
          Q_DRK_SOJU_CC*volume[4]*ach_vol[4]*0.785/100+
          Q_DRK_BEER_SHOT*volume[5]*ach_vol[5]*0.785/100 +
...
gq_18[, DRK_gram := DRK_onetime*PER*Q_DRK_FRQ]
gq_18[, DRK_freq := PER*Q_DRK_FRQ]

gq_1922[, DRK_gram := DRK_onetime*PER*Q_DRK_FRQ]
gq_1922[, DRK_freq := PER*Q_DRK_FRQ]



# 어떻게 하던지 결과 테이블이 a라고 가정
...
# dementia.med.raw <- rbindlist(
#   list(
#     t30[
#       MCARE_DIV_CD_ADJ %in% unlist(code.dementia.drug),
#       .(
#         CMN_KEY = as.character(CMN_KEY),
#         MCARE_DIV_CD_ADJ,
#         Type_drug = code.dementia.drug.named[MCARE_DIV_CD_ADJ]
#       )
#     ],
#     t60[
#       MCARE_DIV_CD_ADJ %in% unlist(code.dementia.drug),
...
# dementia.med.raw <- read_fst("data/t30_60_dimentia.fst", as.data.table = T)
